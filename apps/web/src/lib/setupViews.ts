...
export async function ensurePublicViews() {
  ...
  try {
    ...
    // Create/replace views with sanitised public fields
    await client.query(`
      CREATE OR REPLACE VIEW public_api.v_pilots AS
      SELECT id, ref, title, status, mission, summaryPublic, startDate, endDate, totalBudget,
             exportPotential, publishedAt
      FROM "Pilot"
    ...
    // Grant select to reader role if present
